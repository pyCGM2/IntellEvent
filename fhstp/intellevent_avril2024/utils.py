...
# Function: 'fc_pred'
    ...
    # Input:
        ...
        # cam_frequency: is the capturing frequency of the current trial
...
def fc_pred(rs_velo, x_traj, subject_name, start_frame, vicon, cam_frequency):
    fc_preds = requests.post('http://127.0.0.1:5000/predict_fc', json={'traj': rs_velo})
    fc_preds = np.array(fc_preds.json())[0]
    set_ic_events(fc_preds, x_traj[0,:], x_traj[3,:], subject_name, start_frame, vicon, cam_frequency)
    return True
...
def fo_pred(rs_velo, x_traj, subject_name, start_frame, vicon, cam_frequency):
    ...
    set_fo_events(fo_preds, x_traj[0,:], x_traj[3,:], subject_name, start_frame, vicon, cam_frequency)
    return True
...
# Function: 'set_ic_events'
    ...
    # It then creates an event using the vicon. CreateAnEvent() function of the viconnexus API object,
...
def set_ic_events(fc_preds, l_heel, r_heel, subject_name, start_frame, vicon, cam_frequency):
    [loc, height] = find_peaks(fc_preds[:, 1], height=min_peak_threshold, distance=25)
    loc = np.ceil( (loc / base_frequency) * cam_frequency)
    for fc in loc:
        if l_heel[int(fc)] < r_heel[int(fc)]:
            vicon.CreateAnEvent(subject_name, "Left", "Foot Strike", int(fc + start_frame), 0.0)
        else:
            vicon.CreateAnEvent(subject_name, "Right", "Foot Strike", int(fc + start_frame), 0.0)
...
# Function: 'set_fo_events'
    ...
    # Output:
        # None - This function returns only True or False indicating whether the function executed successfully.
...
def set_fo_events(fc_preds, l_heel, r_heel, subject_name, start_frame, vicon, cam_frequency):
    [loc, height] = find_peaks(fc_preds[:, 1], height=min_peak_threshold, distance=25)
    loc = np.ceil( (loc / base_frequency) * cam_frequency)
    for fo in loc:
        if l_heel[int(fo)] > r_heel[int(fo)]:
...
def get_trial_infos(c3d_trial):
    #grf_frequency = c3d_trial['parameters']['ANALOG']['RATE']['value'][0]
...
def resample_data(traj, sample_frequ, frequ_to_sample):
    ...
    resampled_data = [pd.DataFrame(val, index=index).resample('{}N'.format(int(1e9 / frequ_to_sample))).mean() for val in traj]
